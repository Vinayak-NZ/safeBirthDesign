...
names(app_v2_completed_con) <- 
  sub("_t0$", "_pre", names(app_v2_completed_con))

names(app_v2_completed_con) <- 
  sub("_t1$", "_post", names(app_v2_completed_con))
...
names(app_v2_completed_int) <- 
  sub("_t0$", "_pre", names(app_v2_completed_int))

names(app_v2_completed_int) <- 
  sub("_t4$", "_post", names(app_v2_completed_int))

names(app_v2_completed_int) <- 
  sub("_t2$", "_post", names(app_v2_completed_int))

app_v2_imputed <- rbind(app_v2_completed_con, 
                        app_v2_completed_int)


pre_vars <- 
  grep("_pre$", names(app_v2_imputed), value = TRUE)

app_v2_imputed[pre_vars] <- lapply(
  app_v2_imputed[pre_vars],
  function(x) {
    x[x == 0] <- 1            
    ((x - 1) / 5) * 100      
  }
...
app_v2_imputed$comm_mean_pre <- rowMeans(
  app_v2_imputed[, paste0("comm", 1:7, "_pre")],
  na.rm = TRUE
)

app_v2_imputed$comm_mean_post <- rowMeans(
  app_v2_imputed[, paste0("comm", 1:7, "_post")],
  na.rm = TRUE
)

app_v2_imputed$safe_mean_pre <- rowMeans(
  app_v2_imputed[, paste0("safe", 1:2, "_pre")],
  na.rm = TRUE
)

app_v2_imputed$safe_mean_post <- rowMeans(
  app_v2_imputed[, paste0("safe", 1:2, "_post")],
  na.rm = TRUE
)
